...
int main() {
  std::cout << "Testing CPU SLLS Impute..." << std::endl;
  const int N = 4, D = 3;
  // Data with missing entries in column 2 (index 2) for first row
  float X_orig[N * D] = {1.0f, 2.0f, 0.0f, 2.0f, 3.0f, 4.0f,
                         3.0f, 5.0f, 6.0f, 4.0f, 6.0f, 8.0f};
  ...
  if (X_test[0 * D + 2] != 0.0f) {
    std::cout << "SUCCESS: SLLS imputed value." << std::endl;
  } else {
    std::cout << "FAILURE: SLLS did not impute." << std::endl;
  }
  return 0;
}
