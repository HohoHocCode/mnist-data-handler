...
int main() {
    std::string dir = R"(D:\icu_dataset\processed\cuda)";
    auto meta = read_meta_json(dir + "\\meta.json");

    size_t n = (size_t)meta.rows * (size_t)meta.cols;
    auto X = read_f32(dir + "\\" + meta.x_file, n * sizeof(float));
    auto M = read_u8(dir + "\\" + meta.m_file, n * sizeof(uint8_t));

    std::cout << "Loaded rows=" << meta.rows << " cols=" << meta.cols << "\n";
    std::cout << "X bytes=" << X.size() * sizeof(float) << " M bytes=" << M.size() << "\n";
    std::cout << "features=" << meta.features.size() << "\n";
    return 0;
}
