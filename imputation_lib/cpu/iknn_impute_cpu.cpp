...
class IkNNImputerCpu : public IImputer {
public:
  IkNNImputerCpu(int k = 2, int iterations = 5) : K(k), max_iters(iterations) {}

  std::string name() const override {
    return "IkNNImputerCpu (K=" + std::to_string(K) + ")";
  }

  void impute(float *X, const std::uint8_t *Mask, int N, int D) override {
    std::vector<float> X_cur(X, X + N * D);
    for (int iter = 0; iter < max_iters; ++iter) {
      std::vector<float> X_next = X_cur;
      for (int i = 0; i < N; ++i) {
        for (int j = 0; j < D; ++j) {
          if (Mask[i * D + j] == 0) {
